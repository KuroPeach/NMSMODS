...
EXOMATERIALISER_ON_PLANETBASE = true --Vanilla false // Mod default false // true to enable the Orbital Exocraft Materialiser on planet bases (decorative purpose only), ALL_PARTS_ON_PLANETBASE must be true too
...
FARM_IN_ANY_BIOME = true             --Vanilla false // Mod default false // true to enable planting in any biome
METAL_PARTS_OUTSIDE_BASE = true      --Vanilla false // Mod default false // true to makes some metal parts buildable outside of bases. Won't snap together outside of bases! Use it at your own risks!
GEOBAYS_ON_FREIGHTER = true          --Vanilla false // Mod default false // true to enable vehicles geobays on freighters, ALL_PARTS_ON_FREIGHTER must be true too. Can be very glitchy, use it at your own risks!
BASESTORAGE_ON_FREIGHTER = true      --Vanilla false // Mod default false // true to enable base storage containers on freighters, ALL_PARTS_ON_FREIGHTER must be true too. Can be very glitchy, use it at your own risks!
O2_ATMO_HARVESTERS_ANYWHERE = true   --Vanilla false // Mod default false // true to make Oxygen and Atmosphere harvesters buildable NOT only on bare terrain (also makes them buildable but bugged on dead planets).
CAN_SCALE_PREFAB_PARTS = true        --Vanilla false // Mod Default false // true to allow all prefab rooms related parts to be scaled (doors attachment points bug out when scaled though)
CAN_SCALE_EXTRACTORS = true          --Vanilla false // Mod Default false // true to allow gas/mineral extractors to be scaled (when greatly scaled their resources won't be linked to the resources network though)
...
NPCSTATION_ON_FREIGHTER = true       --Vanilla false // Mod default false // true to enable NPC stations on freighters, ALL_PARTS_ON_FREIGHTER must be true too. Only enable if main story completed.
...
GEOBAYS_ON_FREIGHTER = GUIF(
{ false, [[Do you want GeoBays on Freighters?  Default = N.  Press ENTER for default value.]] }, 10)
print("GEOBAYS_ON_FREIGHTER = " .. tostring(GEOBAYS_ON_FREIGHTER))

FARM_IN_ANY_BIOME = GUIF({ false, [[Do you want to farm in any biome?  Default = N.  Press ENTER for default value.]] },
    10)
print("FARM_IN_ANY_BIOME = " .. tostring(FARM_IN_ANY_BIOME))

BASESTORAGE_ON_FREIGHTER = GUIF(
{ false, [[Do you want to place Base Storage Containers on Freighters?  Default = N.  Press ENTER for default value.]] },
    10)
print("BASESTORAGE_ON_FREIGHTER = " .. tostring(BASESTORAGE_ON_FREIGHTER))

FREIGHTERROOMS_ON_PLANETBASE = GUIF(
{ false,
    [[Do you want to place Freighter Tech and Bio rooms on Planet Bases?  Default = N.  Press ENTER for default value.]] },
    10)
...
while Multiplier do
    ...
    if Multiplier < minChoice or Multiplier > maxChoice then
        print("         >>> [" ..
        Multiplier ..
        "] is NOT a valid choice.  Must be a number between " .. minChoice .. "-" .. maxChoice .. ".  Please retry! <<<")
    else
        break
    end
end
...
if DISABLE_TERRAINEDIT_FOR_ALL == false then
    ...
    if DISABLE_TERRAINEDIT_FOR_TIMBER then
        for _, v in ipairs(TIMBER_FLOOR_KEYWORD_TABLE) do
            n = n + 1;
            DISABLE_TERRAINEDIT_KEYWORD_TABLE[n] = v
        end
    end
    if DISABLE_TERRAINEDIT_FOR_ALLOY then
        for _, v in ipairs(ALLOY_FLOOR_KEYWORD_TABLE) do
            n = n + 1;
            DISABLE_TERRAINEDIT_KEYWORD_TABLE[n] = v
        end
    end
    if DISABLE_TERRAINEDIT_FOR_STONE then
        for _, v in ipairs(STONE_FLOOR_KEYWORD_TABLE) do
            n = n + 1;
            DISABLE_TERRAINEDIT_KEYWORD_TABLE[n] = v
        end
    end
    if DISABLE_TERRAINEDIT_FOR_WOOD then
        for _, v in ipairs(WOOD_FLOOR_KEYWORD_TABLE) do
            n = n + 1;
            DISABLE_TERRAINEDIT_KEYWORD_TABLE[n] = v
        end
    end
    if DISABLE_TERRAINEDIT_FOR_CONCRETE then
        for _, v in ipairs(CONC_FLOOR_KEYWORD_TABLE) do
            n = n + 1;
            DISABLE_TERRAINEDIT_KEYWORD_TABLE[n] = v
        end
    end
    if DISABLE_TERRAINEDIT_FOR_METAL then
        for _, v in ipairs(METAL_FLOOR_KEYWORD_TABLE) do
            n = n + 1;
            DISABLE_TERRAINEDIT_KEYWORD_TABLE[n] = v
        end
    end
    ...
    if DISABLE_TERRAINEDIT_FOR_LIGHTFLOOR then
        for _, v in ipairs(LIGHT_FLOOR_ID_TABLE) do
            n = n + 1;
            DISABLE_TERRAINEDIT_ID_TABLE[n] = v
        end
    end
    if DISABLE_TERRAINEDIT_FOR_PAVING then
        for _, v in ipairs(PAVING_FLOOR_ID_TABLE) do
            n = n + 1;
            DISABLE_TERRAINEDIT_ID_TABLE[n] = v
        end
    end
...
NOT_FREIGHTER_BUILDPART_ID_TABLE = { "BASE_FLAG", "BASECAPSULE", "MESSAGE", "BUILDLANDINGPAD", "S_LANDINGZONE",
    "BUILDSIGNAL", "BUILDBEACON", "U_EXTRACTOR_S", "U_GASEXTRACTOR", "U_SILO_S", "U_GENERATOR_S", "U_PIPELINE",
    "CREATURE_FEED", "CREATURE_FARM", "BUILDHARVESTER", "BUILDGASHARVEST", "TELEPORTER", "O2_HARVESTER" }
...
FREIGHTERROOMS_ON_PLANETBASE_ID_TABLE = { "FRE_ROOM_FLEET", "FRE_ROOM_SCAN", "FRE_ROOM_TELEPO", "FRE_ROOM_DRESS",
    "FRE_ROOM_NPCSCI", "FRE_ROOM_NPCBUI", "FRE_ROOM_SHOP", "FRE_ROOM_TECH", "FRE_ROOM_PLANT1", "FRE_ROOM_PLANT0",
    "FRE_ROOM_COOK", "FRE_ROOM_NPCFAR", "FRE_ROOM_BIO", "FRE_ROOM_NPCWEA" }
...
if O2_ATMO_HARVESTERS_ANYWHERE then
    for _, v in ipairs(O2_ATMO_HARVESTERS_ID_TABLE) do
        table.insert(ANYTERRAIN_BUILDPART_ID_TABLE, v)
    end
end
...
MISC_ON_FREIGHTER_ID_TABLE = { "POWERLINE_HIDER", "NOISEBOX", "SPAWNER_BALL", "BYTEBEATSWITCH", "RACE_START", "RACE_RAMP",
    "RACE_BOOSTER", "BUILD_REFINER2", "BUILD_REFINER3", "DRESSING_TABLE", "BUILDTERMINAL", "TELEPORTER", "BUILDSIGNAL",
    "O2_HARVESTER", "BUILDGASHARVEST", "BUILDHARVESTER", "BUILDANTIMATTER", "BASECAPSULE", "CREATURE_FARM",
    "CREATURE_FEED", "U_GENERATOR_S", "U_PIPELINE", "U_EXTRACTOR_S", "U_SILO_S", "U_GASEXTRACTOR", "MESSAGEMODULE",
    "BUILDLANDINGPAD", "S_LANDINGZONE", "BASECAPSULE" }

-- Plant in any biome
FARM_IN_ANY_BIOME_ID_TABLE = { "RADIOPLANT", "TOXICPLANT", "SNOWPLANT", "SACVENOMPLANT", "SCORCHEDPLANT", "POOPPLANT",
    "GRAVPLANT", "CREATUREPLANT", "BARRENPLANT", "LUSHPLANT", "PEARLPLANT", "NIPPLANT" }
...
if CAN_SCALE_EXTRACTORS == false then
    for _, v in ipairs(EXTRACTORS_ID_TABLE) do
        table.insert(NOT_SCALEABLE_BUILDPART_ID_TABLE, v)
    end
end
...
UNLIMITED_BUILPART_ID_TABLE = { "BASE_TERRARIUM", "BASE_AQUARIUM", "BASE_TOYSPHERE", "BASE_TOYCORE", "DRESSING_TABLE",
    "SPAWNER_BALL", "U_SWITCHBUTTON", "BYTEBEAT", "MESSAGEMODULE", "BUILDTERMINAL", "RACE_RAMP", "RACE_BOOSTER",
    "BASE_ROBOTOY", "BASE_TOYCUBE", "BUILDLANDINGPAD", "S_LANDINGZONE", "TELEPORTER", "TELEPORTER_F", "RACE_START",
    "CHECKPOINT", "WATERBUBBLE", "BUILD_REFINER2", "BUILD_REFINER3", "BASE_TOYJELLY", "BLD_PLANET_HOLO",
    "BUILDSEAHARVEST", "BLD_DATASIGN" }
